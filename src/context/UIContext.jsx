...
export function UIProvider({ children }) {
  ...
  const [panelNuevoActivo, setPanelNuevoActivo] = useState('menu'); // menu | calendarizacion | email | quorum | sync
  const [sidebarTerciarioAbierto, setSidebarTerciarioAbierto] = useState(false);
  const [terminoBusqueda, setTerminoBusqueda] = useState('');
  const [modalActivo, setModalActivo] = useState(null); // null | 'acta' | 'nuevoProyecto' | 'preview' | 'adjuntar'
